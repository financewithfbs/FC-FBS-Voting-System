...
export async function GET(req: Request) {
  try {
    ...
    const debateResults = await Promise.all(debates.map(async (debate) => {
      const teamsWithScores = await Promise.all(debate.teams.map(async (dt) => {
        // Get panelist scores for this team in this debate
        const panelistScores = debate.panelistScores.filter(ps => ps.teamId === dt.teamId)
        ...
        const audienceVotes = debate.votes.filter(v => v.teamId === dt.teamId).length
    ...
    return NextResponse.json(
      { error: "Error calculating results: " + (error instanceof Error ? error.message : "Unknown error") },
      { status: 500 }
    )
  }
}